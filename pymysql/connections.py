...
class Connection(object):
    ...
    def close(self):
        send_data = struct.pack('<l',1) + COM_QUIT
        sock = self.socket
        sock.send(send_data)
        sock.close()
...
class MySQLResult(object):
    ...
    def _get_description(self):
        ...
        for i in range(self.field_count):
            ...
            self.position += 2
            #length
            length = struct.unpack('<l', data[self.position:self.position+4])
            self.position += 4
    ...
    def _seek_and_get_string(self):
        len = self._get_field_length()
        if len : 
            str = self.data[self.position:self.position+len]
            self.position += len
            return str
        return None
    ...
    def _get_field_length(self, longlong=False):
        ...
        if c < UNSIGNED_CHAR_COLUMN:
            ...
            len = struct.unpack('<H', data[pos:pos+UNSIGNED_SHORT_LENGTH])
